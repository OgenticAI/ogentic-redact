...
fn test_privilege_pack_loads_and_validates() {
    ...
    assert!(
        ...
        "privilege pack must have precedence group"
    );
    assert!(
        !pack.entities.is_empty(),
        "privilege pack must have entity definitions"
    );
    ...
    for entity in &pack.entities {
        ...
        for pattern in &entity.patterns {
            assert!(!pattern.regex.is_empty(), "pattern regex must not be empty");
        }
    }
}
...
fn test_phi_pack_loads_and_validates() {
    ...
    assert!(
        !pack.precedence_group.is_empty(),
        "PHI pack must have precedence group"
    );
    assert!(
        !pack.entities.is_empty(),
        "PHI pack must have entity definitions"
    );

    let diagnosis_exists = pack
        .entities
        .iter()
        .any(|e| e.entity_type.to_uppercase().contains("DIAGNOSIS"));
    assert!(diagnosis_exists, "PHI pack must contain diagnosis entity");

    let treatment_exists = pack
        .entities
        .iter()
        .any(|e| e.entity_type.to_uppercase().contains("TREATMENT"));
    assert!(treatment_exists, "PHI pack must contain treatment entity");
}
...
fn test_mnpi_pack_loads_and_validates() {
    ...
    let earnings_exists = pack
        .entities
        .iter()
        .any(|e| e.entity_type.to_uppercase().contains("EARNINGS"));
    assert!(
        earnings_exists,
...
fn test_all_packs_have_unique_precedence_groups() {
    ...
    let mut precedence_groups: Vec<String> =
        packs.iter().map(|p| p.precedence_group.clone()).collect();
...
fn test_pack_entity_types_are_unique_within_pack() {
    ...
    for pack in packs {
        let mut entity_types: Vec<String> = pack
            .entities
            .iter()
...
fn test_phi_pack_has_medical_entities() {
    ...
    assert!(
        entity_types
            .iter()
            .any(|t| t.to_uppercase().contains("MEDICATION")
                || t.to_uppercase().contains("PRESCRIPTION")),
...
fn test_recognizers_are_valid_builtin_types() {
    ...
    let valid_types = ["builtin", "custom"];
